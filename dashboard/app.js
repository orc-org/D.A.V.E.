...
const cameraStreamNames = ['csi0', 'csi1', 'usb'];
const cameraTopics = {
    'csi0': '/camera/csi_0/image_raw',
    'csi1': '/camera/csi_1/image_raw',
    'usb': '/camera/usb_back/image_raw'
};

function initCameraStreams() {
    cameraStreamNames.forEach(name => {
        const img = document.getElementById(`img-feed-${name}`);
        const placeholder = document.getElementById(`placeholder-${name}`);
        const status = document.getElementById(`status-${name}`);
        const btn = document.getElementById(`btn-toggle-cam-${name}`);
        const card = document.getElementById(`feed-card-${name}`);
        ...
        if (img) {
            img.onload = () => {
                ...
                if (status) {
                    ...
                    status.className = "feed-status online";
                }
            };
            ...
            img.onerror = () => {
                ...
                if (status) {
                    ...
                    status.className = "feed-status offline";
                }
            };
        }
        ...
        if (btn && card) {
            btn.addEventListener('click', () => {
                ...
                if (isActive) {
                    btn.classList.remove('neon-btn-red');
                    btn.classList.add('neon-btn-blue');
                    card.classList.remove('hidden');
                    // Immediately try to load if active
                    loadSingleStream(name);
                } else {
                    btn.classList.remove('neon-btn-blue');
                    btn.classList.add('neon-btn-red');
                    card.classList.add('hidden');
                    // Stop load to save bandwidth
                    if (img) img.src = "";
                }
            });
        }
    ...
    // Start periodic retry/refresh check
    if (!cameraRetryInterval) {
        cameraRetryInterval = setInterval(retryOfflineStreams, 5000);
    }
}

function loadSingleStream(name) {
    const img = document.getElementById(`img-feed-${name}`);
    const card = document.getElementById(`feed-card-${name}`);
    if (!img || !card || card.classList.contains('hidden')) return;

    if (connected && ros) {
        // Extract host from rosbridge url
        const host = getHostFromRosUrl(ipInput.value);
        const topic = cameraTopics[name];
        const newSrc = `http://${host}:8080/stream?topic=${topic}&width=640&height=360`; // downscaled slightly for dashboard display
        
        // Only set if different or currently empty/offline
        if (img.src !== newSrc || img.classList.contains('hidden')) {
            console.log(`Starting stream for ${name} pointing to ${newSrc}`);
            img.src = newSrc;
        }
    } else {
        stopStreams();
...
function retryOfflineStreams() {
    if (!connected) return;
    cameraStreamNames.forEach(name => {
        const img = document.getElementById(`img-feed-${name}`);
        const card = document.getElementById(`feed-card-${name}`);
        // If it is active (not hidden card) but current image is hidden (offline), retry loading it
        if (card && !card.classList.contains('hidden') && img && img.classList.contains('hidden')) {
            loadSingleStream(name);
        }
    });
}

function getHostFromRosUrl(url) {
    try {
        const temp = url.replace('ws://', '').replace('wss://', '');
        const hostPort = temp.split('/')[0];
        return hostPort.split(':')[0] || 'localhost';
    } catch (e) {
        return 'localhost';
    }
}

// Call camera streams initialization
initCameraStreams();
